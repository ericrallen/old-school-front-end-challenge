...
import { court, APPOINTMENT_LENGTH } from "./index";
...
function getRandomNames(count: number, exclude: string[] = []) {
...
function calculateExpectedWaitTime(nameIndex: number, judges: number) {
...
test("Validation Tests", () => {
  ...
  expect(calibrationTestOne).toBe(60);
  expect(calibrationTestTwo).toBe(150);
});
...
test("Utility Calibration Tests", () => {
  const tests = [
    {
      name: "Jules",
      judges: 3,
  ...
  tests.forEach((test) => {
    expect(getNameIndex(test.name, test.names)).toBe(test.nameIndex);
    expect(calculateExpectedWaitTime(test.nameIndex, test.judges)).toBe(
      test.expected
    );
  });
});
...
test("should calculate the correct time for a single person", () => {
  const result = court(PHONETIC_ALPHABET[0], 1, "");

  expect(result).toBe(APPOINTMENT_LENGTH);
});

test("should calculate the correct time for a large number of people", () => {
  const name = "John";
  const judges = 2;
  const names = PHONETIC_ALPHABET;
  ...
  expect(result).toBe(APPOINTMENT_LENGTH * 5);
});
...
test("should calculate the correct time for a large number of people", () => {
  ...
  expect(result).toBe(APPOINTMENT_LENGTH * 13);
});
...
test("should calculate the correct time for a large number of people", () => {
  ...
  // Zombie should be near the end of the alphabet, just before Zulu
  expect(result).toBe(APPOINTMENT_LENGTH * 26);
});

test("should calculate the correct time for two people with one judge", () => {
...
test("should calculate the correct time for multiple people with one judge", () => {
  const name = getRandomNames(1)[0];
  const judges = 1;
...
test("should calculate the correct time for multiple people with two judges", () => {
  const name = getRandomNames(1)[0];
  const judges = 2;
...
test("should calculate the correct time for multiple people with three judges", () => {
  const name = getRandomNames(1)[0];
  const judges = 3;
...
for (let i = 0; i < NUMBER_OF_TESTS; i++) {
  test("should calculate the correct time for any random case", () => {
    const name = getRandomNames(1)[0];
